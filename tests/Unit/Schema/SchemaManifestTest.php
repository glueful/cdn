...
namespace Glueful\Extensions\Cdn\Tests\Unit\Schema;
...
final class SchemaManifestTest extends TestCase
{
    ...
    {
        $base = sys_get_temp_dir() . '/cdn-mf-' . uniqid('', true);
        ...
        self::assertSame([], $manifest->migrationDescriptors()['glueful/cdn']);
        self::assertNotContains('glueful/cdn', $manifest->undeclaredGluefulPackages());
    }

    public function testResolvesWhenEnabledAloneAtItsDeclaredFloor(): void
    {
        $g = $this->manifest();
        $candidates = ['glueful/cdn' => new \Glueful\Extensions\ExtensionCandidate(
            name: 'glueful/cdn',
            provider: $g['provider'],
