...
namespace Glueful\Extensions\Cdn\Console;
 ...
 * @package Glueful\Extensions\Cdn\Console
 */
#[AsCommand(
    name: 'cache:purge',
...
{
    private EdgeCachePurger $edgeCacheService;
    ...
    {
        $this->edgeCacheService = $this->getService(EdgeCachePurger::class);
    }
    ...
    private function purgeBatch(string $batchFile, bool $dryRun): int
    {
        ...
        if ($lines === false) {
            $this->io->error("Failed to read batch file: {$batchFile}");
            return self::FAILURE;
        }
        if ($lines === []) {
    ...
    private function displayCacheStats(string $phase): void
    {
        ...
        try {
            ...
            // TODO(R4): pre-existing getStats() key mismatch — out of scope for the extraction
